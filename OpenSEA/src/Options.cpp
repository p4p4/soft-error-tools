...
bool Options::parse(int argc, char **argv)
{
	...
	for (int arg_count = 1; arg_count < argc; ++arg_count)
	{
		...
		else if (arg == "-env")
		{
			...
			if (arg_count >= argc)
			{
				cerr << "Option -env must be followed by a filename." << endl;
				return true;
			}
			aig_env_file_name_ = string(argv[arg_count]);
		}
		...
		else if (arg == "-p")
		{
			...
			if (arg_count >= argc)
			{
				cerr << "Option -p must be followed by a string indicating what to print."
						<< endl;
				return true;
			}
...
void Options::printHelp() const
{
	...
	cout << "                         Finds vulnerabilities using a test-case" << endl;
	...
	cout << "                         Finds false positives using a test-case" << endl;
	cout << "                         Adoptions of the sta and stla algorithms to" << endl;
	cout << "                         find false positives (instead of vulnerabilities)" << endl;
	cout << "                 bdd:  The Binary Decision Diagram analysis" << endl;
	cout << "                         Finds vulnerabilities using a test-case" << endl;
	cout << "                         Similar to 'sta' and 'stla', but using BDDs" << endl;
	cout << "                         instead of SAT-solvers" << endl;
	cout << "                 dp:   The Definitely Protected latches analysis" << endl;
	cout << "                         Finds definitely protected latches" << endl;
	...
	cout << "                      to leave some or all values in the given TestCase" << endl;
	cout << "                      open (write '?' instead of '0' or '1')" << endl;
	cout << "                   3: SYMB_TIME_LOCATION_INPUTS - as mode 1, but allows" << endl;
	cout << "                      to leave some or all values in the given TestCase" << endl;
	cout << "                      open (write '?' instead of '0' or '1')" << endl;
	cout << "                 Back-end 'bdd': " << endl;
	cout << "                 Back-end 'dp': " << endl;
